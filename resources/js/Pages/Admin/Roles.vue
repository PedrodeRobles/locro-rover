    ...
    <div class="bg-gray-800 text-white pt-24">
        ...
        <div class="flex justify-center md:justify-end items-center pb-6 md:px-4">
            <div class="flex space-x-4">
                <Link :href="route('admin.users.soft-deleted')" class="flex items-center rounded-md bg-red-600 hover:bg-red-500 cursor-pointer p-2">
...
async function softDelete(user) {
    try {
        if (confirm(`¿Estás seguro que queres borrar logicamente a ${user.name}?
Podras recuperar este usuario en 'Rovers inactivos'`)) {
            await router.put(`/admin/users/${user.id}/softDelete`);
            await toast.success(`Rover eliminado con éxito!`, { autoClose: 2500 });
        }
    } catch (error) {
        console.error('Error al realizar la operación:', error);
        await toast.error(`Error al eliminar rover: ${user.name}`, { autoClose: 2500 });
    }
}
</script>